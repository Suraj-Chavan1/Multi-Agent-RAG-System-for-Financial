# 🚀 Vercel Deployment Script for App Folder Only
# Run this script from the app directory

Write-Host "🚀 Starting Vercel Deployment for Financial RAG System..." -ForegroundColor Green

# Check if we're in the app directory
if (!(Test-Path "main.py")) {
    Write-Host "❌ Error: Please run this script from the app directory" -ForegroundColor Red
    Write-Host "📁 Navigate to: cd app" -ForegroundColor Yellow
    exit 1
}

Write-Host "✅ Found main.py - proceeding with deployment" -ForegroundColor Green
...
Write-Host "   - POST https://your-app.vercel.app/query" -ForegroundColor White
Write-Host "   - POST https://your-app.vercel.app/upload" -ForegroundColor White
